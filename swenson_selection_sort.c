 ...
 * 
 * Description:
 * This file provides a MATLAB MEX interface for the `selection_sort` function,
 * provided in the `sort.h` library. It demonstrate for MATLAB users how to
 * utilize the `selection_sort` function for sorting arrays of type `double`.
 * 
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1)
    {
        mexErrMsgIdAndTxt("MATLAB:swenson_selection_sort:invalidNumInputs", "One input required.");
    }
    if (nlhs != 1)
    {
        mexErrMsgIdAndTxt("MATLAB:swenson_selection_sort:maxlhs", "One output required.");
    }

    // Check if the input is a double column vector
    if (!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]))
    {
        mexErrMsgIdAndTxt("MATLAB:swenson_selection_sort:notDouble",
                          "Input matrix must be type double.");
    }
    if(mxGetN(prhs[0])!=1) {
        mexErrMsgIdAndTxt("MATLAB:swenson_selection_sort:notColVector","Input must be a column vector.");
    }
    ...
    swenson_selection_sort(dst, size);
}